...
// 同时修改 EPD_SendData_DMA 函数的“后续处理”（主函数中调用时）：
// 调用 EPD_SendData_DMA 后，在主函数中等待传输完成，并处理CS和BUSY：
void EPD_SendData_DMA(const uint8_t *data, uint16_t len) 
{
    DMA_Cmd(EPD_DMA_STREAM, DISABLE);
    EPD_DMA_STREAM->M0AR = (uint32_t)data;
    EPD_DMA_STREAM->NDTR = len;

    EPD_DC_Set(); 
    EPD_CS_Clr();

    DMA_ClearFlag(EPD_DMA_STREAM,EPD_SPI_TX_DMA_FLAG);
    g_epd_dma_tx_done = 0;
    DMA_Cmd(EPD_DMA_STREAM, ENABLE);

    // 【新增】主函数内等待传输完成（替代中断内的等待）
    while(g_epd_dma_tx_done == 0); 
    // 等待SPI硬件完成最后一字节发送（此时在主函数，不占用中断资源）
...
void DMA1_Stream4_IRQHandler()
{
    ...
    {
        DMA_ClearITPendingBit(EPD_DMA_STREAM,EPD_SPI_TX_DMA_IT); // 仅清标志
        DMA_Cmd(EPD_DMA_STREAM, DISABLE); // 关闭DMA（普通模式必需）
        g_epd_dma_tx_done = 1; // 仅置完成标志，其他操作移到主函数
    }
}
