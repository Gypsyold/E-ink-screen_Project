#include "stm32f4xx.h"                  // Device header
#include "board.h"
#include "bsp_uart.h"
#include <stdio.h>
#include "EPD_GUI.h"
#include "spi_w25Q128_flash.h"
#include "GB2312_kaiti_16.h"





// 写字库函数（可直接放在main函数中调用，或单独封装）
void Write_GBK_Font_To_W25Q128(void)
{
    // 先初始化SPI和W25Q128（必须先初始化，否则无法通信）

   printf("W25Q128芯片ID是%x\r\n",W25Q128_readID());

#if 0
	//测试
	printf("开始测试写入...\r\n");
	W25Q128_ContinuousWrite((uint8_t *)test,W25Q128_GBK_ADDR,sizeof(test));
	printf("字库测试写入完成 ！！\r\n");
	
#endif	
	
	
#if 1  // ========== 第一次：写入字库上半部分（4418个汉字，141376字节） ==========
    printf("开始写入字库上半部分...\r\n");
    W25Q128_ContinuousWrite
	(
        (uint8_t *)m_char_bits,     // 写入起始地址：字库开头（0x000000）
        W25Q128_GBK_ADDR,           // 数据指针：数组第0个汉字（上半段起始）
        sizeof(m_char_bits)         // 写入长度
    );
    printf("字库上半部分写入完成！\r\n");
#endif

#if 0 // ========== 第二次：写入字库下半部分（4418个汉字，141376字节） ==========
    printf("开始写入字库下半部分...\r\n");
    W25Q128_WriteData(
        W25Q128_GBK_ADDR + GBK_FONT_PART1_LEN,  // 写入起始地址：衔接上半段（0x000000+141376=0x22800）
        (uint8_t *)(&m_char_bits[GBK_PART1_CHAR_NUM]),  // 数据指针：数组第4418个汉字（下半段起始）
        GBK_FONT_PART2_LEN                          // 写入长度：141376字节
    );
    printf("字库下半部分写入完成！长度：%d 字节\r\n", GBK_FONT_PART2_LEN);
    printf("完整GBK字库（%d个汉字）写入W25Q128成功！\r\n", GBK_CHAR_NUM);

#endif
}

void Verify_GBK_Font(void)
{
    uint8_t read_buff[32];  // 读取缓冲区（32字节=1个汉字）


    // 1. 校验字库开头（地址0x000000，对应m_char_bits[0]）
    W25Q128_read(read_buff, W25Q128_GBK_ADDR + 32 * 128, 32);
    for(uint8_t i=0; i<32; i++)
    {  
		printf("实际0x%02X\r\n",read_buff[i]);
    }

}

u8 ImageBW[5624];
int main(void)
{

	board_init();
	uart1_init(115200U);
	bsp_spi_flash_init();	
	printf("设备ID是%x\r\n",W25Q128_readID());
	
	//Write_GBK_Font_To_W25Q128();
	//Verify_GBK_Font();
//	for(uint8_t i = 0;i<100;i++)
//	{
//	
//	W25Q128_erase_sector(i);
//	printf("擦除%d扇区\r\n",i);		
//	}
	


	printf("完成\r\n");
	
	EPD_GPIOInit();	
	EPD_Init();
	Paint_NewImage(ImageBW,EPD_W,EPD_H,0,WHITE);		//创建画布
	Paint_Clear(WHITE);	
	EPD_Display_Clear();		
	EPD_ShowString(36,130,"windows",16,BLACK);
	EPD_ShowChinese(0,0,"中景园电子",16,BLACK);
	EPD_ShowChinese(0,16,"电子",24,BLACK);
	
	EPD_ShowChineseString_flash(0,40,"淼",BLACK);
	EPD_Display(ImageBW);
	EPD_Update();
	EPD_DeepSleep();
	

}

