...
/**
 * @brief  GB2312双字节编码→W25Q128地址偏移
 * @param  chn_str: 输入的单个汉字字符串（如"中"，占2字节）
 * @retval 地址偏移（失败返回0xFFFFFFFF）
 */
uint32_t GB2312_to_offset(char *chn_str)
{
//    if (chn_str == NULL) return 0xFFFFFFFF;

    // 1. 提取GB2312双字节编码（高字节GBKH，低字节GBKL）
    uint8_t GBKH = (uint8_t)chn_str[0];
    uint8_t GBKL = (uint8_t)chn_str[1];

    // 2. 校验GB2312编码范围（避免越界）
    if (GBKH < 0xA1 || GBKH > 0xF7 || GBKL < 0xA1 || GBKL > 0xFE)
    {
        return 0xFFFFFFFF;  // 不是GB2312编码
    }

    // 3. 计算地址偏移（GB2312=GBK子集，公式通用）
    uint32_t offset;
    if (GBKL < 0x7F)
    {
...
#define FONT_SIZE_16        16        // 16×16字体
#define FONT_BYTES_16X16    32        // 16×16=32字节/字模

void EPD_show_Chinese_from_flash(u16 x,u16 y,u8 *s,u8 sizey,u16 color)
{
    u8 i, j;
    u16 x0 = x;
    uint8_t GBKH, GBKL;
    uint32_t Addr_offset;  // 地址偏移
    uint8_t pBuff[FONT_BYTES_16X16] = {0};  // 存储读取的32字节字模（不用malloc，栈上分配更简单）

    // 1. 提取GB2312双字节编码（和OLED函数逻辑一致）
    GBKH = *(s);     // 高字节
    GBKL = *(s + 1); // 低字节
	printf("GBKH=%x,GBKL = %x\r\n",GBKH,GBKL);
	
    // 2. 校验编码有效性（避免越界）
    if (GBKH < 0xA1 || GBKH > 0xF7 || GBKL < 0xA1 || GBKL > 0xFE)
    {
        return;  // 不是GB2312编码，不显示
    }

    // 3. 计算地址偏移（完全复用OLED函数的公式，16×16字体直接用）
    if (GBKL < 0x7F)
    {
    ...
    // 4. 从W25Q128读取32字节字模（替换原来的“遍历RAM字库”）
    W25Q128_read(pBuff, W25Q128_GBK_ADDR + Addr_offset * FONT_BYTES_16X16, FONT_BYTES_16X16);

    // 5. 逐点显示（完全保留你原来的显示逻辑，一字未改！）
    for (i = 0; i < FONT_BYTES_16X16; i++)
    {
        for (j = 0; j < 8; j++)
        {
            if (pBuff[i] & (0x01 << j))  // 你的原逻辑：LSB位序，逐点点亮
            {
                Paint_SetPixel(x, y, color);
            }
            x++;
            if ((x - x0) == sizey)  // 一行16个点，换行
            {
                x = x0;
                y++;
                break;
...
/**
 * @brief  扩展：连续显示汉字字符串（可选，方便批量显示）
 * @param  x: 起始x坐标
 * @param  y: 起始y坐标
 * @param  str: 汉字字符串（如"床前明月光"）
 * @param  color: 显示颜色
 * @retval 无
 */
void EPD_ShowChineseString_flash(u16 x, u16 y, u8 *str, u16 color)
{
    u16 curr_x = x;
    // 按“2字节一个汉字”遍历字符串（字符串以'\0'结尾）
    while (*str != '\0' && *(str + 1) != '\0')
    {
        EPD_show_Chinese_from_flash(curr_x, y, str, FONT_SIZE_16, color);
        curr_x += FONT_SIZE_16;  // 每个汉字占16列，偏移x坐标
