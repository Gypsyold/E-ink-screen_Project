...
/**
 * @brief  连续存储函数（自动跨扇区/跨页，仅需传入地址、数据、长度）
 * @param  buffer: 要写入的数据（无类型限制，强制转换为uint8_t*即可）
 * @param  addr: 写入起始地址（W25Q128全地址范围：0x000000~0x1FFFFF）
 * @param  numbyte: 写入数据长度（任意正整数，支持跨扇区）
 * @retval 无
 */
//void W25Q128_ContinuousWrite(uint8_t* buffer, uint32_t addr, uint32_t numbyte) 
//{
//    if (buffer == NULL || numbyte == 0) return;  // 无效参数直接返回

//    uint32_t curr_addr = addr;       // 当前写入地址
//    uint32_t remain_len = numbyte;   // 剩余未写入长度
//    uint8_t* curr_buf = buffer;      // 当前数据指针
//    uint16_t write_len;              // 单次写入长度（≤256字节，不跨页）
...
//	{
//        W25Q128_erase_sector(sec);   // 调用你原有的扇区擦除函数
//        W25Q128_wait_busy();         // 等待擦除完成（必须等，擦除比写入慢）
//    }
...
//	{
//        // 计算当前页还能写多少字节（避免跨页）
//        write_len = W25Q128_PAGE_SIZE - (curr_addr & W25Q128_PAGE_MASK);
//        // 若剩余长度不足一页，取剩余长度
//        if (write_len > remain_len) write_len = (uint16_t)remain_len;

//        // 单次页写入（核心：不跨页，符合硬件规则）
//        W25Q128_write_enable();      // 写使能
//        W25Q128_wait_busy();         // 忙检测

//        W25QXX_CS_ON(0);             // 拉低CS
//        spi_read_write_byte(0x02);   // 页写入指令（0x02）
//        // 发送24位地址
...
//		{
//            spi_read_write_byte(curr_buf[i]);
//        }
//        W25QXX_CS_ON(1);             // 拉高CS
//        W25Q128_wait_busy();         // 等待写入完成
...
void W25Q128_ContinuousWrite(uint8_t* buffer, uint32_t addr, uint32_t numbyte) 
{
    ...
    uint32_t remain_len = numbyte;
    uint8_t* curr_buf = buffer;
    uint32_t write_len;  // 已改成uint32_t，避免溢出
    uint32_t write_count = 0;  // 统计总写入字节数
    ...
    while (remain_len > 0) 
	{
        // 计算单次写入长度（不跨页，最多64字节，进一步降低SPI压力）
        write_len = W25Q128_PAGE_SIZE - (curr_addr & W25Q128_PAGE_MASK);
        if (write_len > remain_len) write_len = remain_len;
        if (write_len > 64) write_len = 64;  // 强制限制单次最多64字节，避免SPI超时
        ...
        spi_read_write_byte((uint8_t)(curr_addr >> 8));
        spi_read_write_byte((uint8_t)curr_addr);
        // 写入数据（添加索引打印，确认数据指针正确）
        for (uint32_t i = 0; i < write_len; i++) 
		{
            spi_read_write_byte(curr_buf[i]);
            // 每写16字节打印一次数据（验证数据没乱）
            if (i % 16 == 0) printf("  偏移%d: 0x%02X ", i, curr_buf[i]);
        }
        printf("\n");
        W25QXX_CS_ON(1);
        W25Q128_wait_busy();
