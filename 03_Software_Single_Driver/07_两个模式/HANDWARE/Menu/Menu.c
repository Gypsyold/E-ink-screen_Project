...
int menu_main(void)
{
	...
	while(1)
	{
		...
		{
			flag_choose_mode ++;
			if(flag_choose_mode == 3){flag_choose_mode = 1;}
		
		}
		if(KeyNum == 2)	// 上一项
		{
			flag_choose_mode --;
			if(flag_choose_mode == 0){flag_choose_mode = 2;}
		...
		if(KeyNum == 3) // 确认
		{
			EPD_Display_Clear();
			EPD_Update();
			return flag_choose_mode;
		}
		
		switch(flag_choose_mode)
		{
			case 1:
			{
				Paint_Clear(WHITE);
			...
			case 2:
			{
				...
				EPD_ShowChinese(23,121,(u8 *)"日历模式",24,BLACK);
				EPD_ShowChinese(171,121,(u8 *)"阅读模式",24,BLACK);	
				EPD_InvertRect(ImageBW,EPD_H/2,0,EPD_H/2,EPD_W);
				EPD_Display(ImageBW);
				EPD_PartUpdate();	
				break;				
			}			
...
int menu2_Motor(void)
{
	...
	uint8_t moflag = 1;
	Paint_Clear(WHITE);
	EPD_ShowString (0,0,(u8 *)"<--                   ",24,BLACK);
	EPD_ShowChinese(0,24,(u8 *)"单次步长             ",24,BLACK);
	EPD_ShowChinese(0,48,(u8 *)"角度控制             ",24,BLACK);
	EPD_ShowChinese(0,72,(u8 *)"速度控制             ",24,BLACK);
	EPD_ShowChinese(0,72,(u8 *)"旋转方向             ",24,BLACK);
	EPD_Display(ImageBW);
	EPD_PartUpdate();
	while(1)
	{
		...
		{
			moflag ++;
			if(moflag == 6){moflag = 1;}
		
		}
		if(KeyNum == 2)	// 上一项
		{
			moflag --;
			if(moflag == 0){moflag = 5;}
		...
		if(menu3_motor == 2){return 0;}
		if(menu3_motor == 3){return 0;}
		if(menu3_motor == 4){return 0;}
		if(menu3_motor == 5){return 0;}
		...
		switch(moflag)
		{
			...
			case 1:
			{
					...
					EPD_ShowChinese(0,48,(u8 *)"角度控制             ",24,BLACK);
					EPD_ShowChinese(0,72,(u8 *)"速度控制             ",24,BLACK);
					EPD_ShowChinese(0,96,(u8 *)"旋转方向             ",24,BLACK);
					EPD_InvertRect(ImageBW,0,0,EPD_H,24);
					EPD_Display(ImageBW);
					EPD_PartUpdate();	
					break;
			...
			case 5:
			{
					...
					EPD_ShowChinese(0,72,(u8 *)"速度控制             ",24,BLACK);
					EPD_ShowChinese(0,96,(u8 *)"旋转方向             ",24,BLACK);
					EPD_InvertRect(ImageBW,0,96,EPD_H,24);
					EPD_Display(ImageBW);
					EPD_PartUpdate();
					break;				
