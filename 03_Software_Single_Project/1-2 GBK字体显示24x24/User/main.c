...
#if 0 // ========== 第二次：写入GBK字库下半部分(11908个字 ，857376字节) ==========
...
//	EPD_ShowChinese(0,16,"电子",24,BLACK);
	...
	while(1)
	{
		...
		EPD_ShowChineseString_flash(0,0,(uint8_t *)"纼绬亓乩乜丂丠乆乊乑乕乗",FONT_24X24,BLACK);

		EPD_ShowChineseString_flash(0,24,(uint8_t *)"垚淼頔",FONT_24X24,BLACK);
		EPD_ShowChineseString_flash(0,2*24,(uint8_t *)"垚淼頔",FONT_24X24,BLACK);
		EPD_ShowMixedString(0,3*24,(uint8_t *)"..淼",FONT_24X24,24,BLACK);
		EPD_ShowMixedString(0,4*24,(uint8_t *)"1.1n頔",FONT_24X24,24,BLACK);
		EPD_ShowMixedString(0,5*24,(uint8_t *)"垚淼頔垚淼頔",FONT_24X24,24,BLACK);	
