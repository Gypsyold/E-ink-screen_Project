...
// 显示任务局部状态（各画面的临时变量）
uint8_t g_MenuSel = 1;        		 // 主菜单选中项（0=日历，1=阅读）- 改为全局供按键任务查询
...
static void Display_Refresh(CurrentScreen screen) 
{
    ...
    switch (screen) 
	{
        ...
        case SCREEN_READING_SEL:
            // 阅读-页码选择
			EPD_ShowChinese(0,0,(u8 *)"红楼梦",24,BLACK);
			EPD_ShowChinese(0,24,(u8 *)"三国演义",24,BLACK);
			EPD_ShowChinese(0,48,(u8 *)"水浒传",24,BLACK);
			EPD_ShowChinese(0,72,(u8 *)"西游记",24,BLACK);
			EPD_InvertRect(ImageBW,0,(g_ReadingPage-1)*24,EPD_H,24);
