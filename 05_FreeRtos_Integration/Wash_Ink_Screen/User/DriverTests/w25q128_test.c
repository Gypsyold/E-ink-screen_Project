 ...
 * 测试参数
 * ================================================================ */
#define TEST_ADDR   0x280000u   /* 测试地址（用户数据区起始，字库区 0x000000~0x27FFFF 禁止写） */
#define TEST_LEN    256u        /* 测试长度（一整页）                          */

/*
  区域               起始地址     结束地址     大小         说明
  ─────────────────────────────────────────────────────────────────
  16×16 字库         0x000000    0x0BAC1F     765 KB      实际数据
    └─ 尾部缓冲                  0x0BFFFF     ~20 KB      5 个扇区空闲
  ─────────────────────────────────────────────────────────────────
  24×24 字库         0x0C0000    0x264F5F    1,683 KB     实际数据
    └─ 尾部缓冲                  0x27FFFF    ~100 KB      24 个扇区空闲
  ─────────────────────────────────────────────────────────────────
  用户数据区         0x280000    0xFFFFFF    ~13.5 MB     SD卡缓存等
  ─────────────────────────────────────────────────────────────────
