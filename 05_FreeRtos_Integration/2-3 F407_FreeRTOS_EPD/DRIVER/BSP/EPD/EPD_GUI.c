...
void EPD_ShowMixedString(u16 x, u16 y, u8 *buf, Font_Size font_size, u16 ascii_size, u16 color)
{
    ...
    // 遍历缓冲区（最多1024字节，避免无限循环）
    while (i < 1024)
    {
        // 终止条件：连续两个0字节（有效数据结束）
        if (buf[i] == 0 && (i+1 >= 1024 || buf[i+1] == 0))
        {
            break;
        }

        // 1. 识别ASCII字符（单字节：0x00~0x7F，排除0）
        if (buf[i] <= 0x7F && buf[i] != 0)
        {
            // 直接调用你提供的原生ASCII单字符函数，保留原生间距
            EPD_ShowChar(curr_x, y, buf[i], ascii_size, color);
            // 间距完全复用你原生逻辑：ASCII间距是size1/2（比如24号字体偏移12px）
            curr_x += ascii_size / 2;
            i++; // ASCII单字节，i+1
        }
        // 2. 识别GBK汉字（双字节：0x81~0xFE开头，下一字节非0）
        else if ((buf[i] >= 0x81 && buf[i] <= 0xFE) && (i+1 < 1024 && buf[i+1] != 0))
        {
            // 直接调用你提供的原生汉字单字函数，保留原生间距
            EPD_show_Chinese_from_flash(curr_x, y, &buf[i], font_size, color);
            // 间距完全复用你原生逻辑：汉字间距是font->width（比如24号字体偏移24px）
            curr_x += font->width;
            i += 2; // GBK双字节，i+2
        }
        // 3. 无效字节，跳过
        ...
        // 换行处理（适配296×152屏幕，超出宽度自动换行）
        if (curr_x >= 296)
        {
            curr_x = x;
            y += (font->height > ascii_size ? font->height : ascii_size); // 按高的字体换行
            if (y >= 152) break; // 超出屏幕高度停止
        }
    }
}
